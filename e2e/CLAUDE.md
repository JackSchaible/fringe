...
Playwright end-to-end tests that run against the live deployed site (`https://fringe.jackschaible.ca` by default). Not part of the PR `test` gate — there's nothing to deploy against until `main` has already shipped, so these only run post-deploy in `deploy.yml`.
