...
**Every new piece of user-facing text or static text-bearing attribute (`alt`, `title`, `aria-label`, etc.) needs an `i18n`/`i18n-*` marker, plus a `<target>` added to `src/locale/messages.fr-CA.xlf` — not just the `lint`-satisfying marker in the template.** `@angular-eslint/template/i18n` catches a missing marker, but nothing catches a missing French `<target>` short of someone reading the diff — don't skip it just because lint passed. Workflow: add the marker → `npx ng extract-i18n` → add the matching `<target>` (next to `<source>`) in `messages.fr-CA.xlf`.
